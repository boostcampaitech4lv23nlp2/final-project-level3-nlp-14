...
class SegformerEfficientSelfAttention(nn.Module):
    ...
    def __init__(self, config, hidden_size, num_attention_heads, sequence_reduction_ratio):
        ...
        self.value = nn.Linear(self.hidden_size, self.all_head_size)
        #self.down_dim = 128
        #self.proj_kv = nn.parameter.Parameter((torch.zeros(self.hidden_size, self.down_dim)), requires_grad=True)
        #self.proj_kv = nn.Linear(self.hidden_size, self.down_dim)
        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)
    ...
    def forward(
        ...
        key_layer = self.transpose_for_scores(self.key(hidden_states))
        value_layer = self.transpose_for_scores(self.value(hidden_states))
        #key_layer = self.proj_kv(self.key(hidden_states)).unsqueeze(1)
        #value_layer = self.proj_kv(self.value(hidden_states)).unsqueeze(1)
        ...
        linformer
        """
        attention_scores = torch.einsum('bhnd, bhkd -> bhnk', query_layer, key_layer)
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        ...
        #context_layer = torch.matmul(attention_probs, value_layer)
        context_layer = torch.einsum('bhnk, bhkd -> bhnd', attention_probs, value_layer)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(new_context_layer_shape)
