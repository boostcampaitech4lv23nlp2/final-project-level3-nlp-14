...
def main(opt):
    ...
    for epoch in range(epochs):
        ...
        train_epoch_loss, train_epoch_miou, train_epoch_lr = train(
            model, train_loader, optimizer, scheduler, num_labels, dev
        )
        val_epoch_loss, val_epoch_miou, val_epoch_miou_by_class = validate(
            model, val_loader, num_labels, category_names, dev
        )
        ...
        logging.info(
            f"Training loss: {train_epoch_loss:.3f} | Training miou: {train_epoch_miou:.3f} | Training lr: {train_epoch_lr}"
        )
        logging.info(
            f"Validation loss: {val_epoch_loss:.3f} | Validation miou: {val_epoch_miou:.3f} | Validation miou by class: {val_epoch_miou_by_class}"
        )
        logging.info("-" * 50)
